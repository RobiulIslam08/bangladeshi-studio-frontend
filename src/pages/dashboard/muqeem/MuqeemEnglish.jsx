...
const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await axios.post("muqeemEnglishUserSubmit", form);
...
if (res.data.status === "success") {
  ...
  return (
    <DashboardLayout title="Muqeem English Form">
      <section className="w-full bg-gray-50 min-h-screen p-4 md:p-6">
                  ...
                  return (
                    <div key={key} className={isLargeField ? "md:col-span-2" : ""}>
                      ...
                      <input
                        ...
                        value={form[key]}
                        onChange={onChange}
                        className="w-full h-11 px-4 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#0B4BFF]/20 transition-all text-sm"
